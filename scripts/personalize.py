...
def main(
    ...
    replacements = [
        ...
        ("ring_attention_jax", package_dir_name),
    ]
    if dry_run:
    ...
    # Rename 'ring_attention_jax' directory to `package_dir_name`.
    if not dry_run:
        (REPO_BASE / "ring_attention_jax").replace(REPO_BASE / package_dir_name)
    else:
        print(f"Renaming 'ring_attention_jax' directory to '{package_dir_name}'")
